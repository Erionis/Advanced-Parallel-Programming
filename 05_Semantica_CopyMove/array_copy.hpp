...
public:
    ...
    */
   Array(Array<T>&& orig)
        : len(orig.len), values(orig.values)
    {
        orig.values = nullptr;
    }
    ...
    */
    ...
    // ha cambiato questa parte rispetto a array.hpp
    /****************************************/
    // Socraccarico operatore di copia permettendo non piu la concatenazione
    // ma la somma vera e propria tra array (elem1 +elem2)
    size_t counter;
    template<typename T>
    Array<T> operator+(const Array<T>& a, const Array<T>& b)
    {
        
        // speriamo che a.size sia uguale a b.size
        Array<T> res{a}; // sto inizializzando res in modo tale che sia una copia di a
    ...
    #ifdef MOVE_ON    // se una certa macro viene definita

    // operatore di somma dove il primo paramentro è un rvalue (&&) cioe il risultato di un espressione
    template<typename T>
    Array<T> operator+( Array<T>&& a, const Array<T>& b)
    {
        counter++; // contatore per vedere quante volte viene eseguito questo operatore+

        for (size_t i = 0; i < b.size(); i++)
        {
            a[i] += b[i]; // non sto creando un nuovo array, ma sto riciclando a per storare la somma
        }
